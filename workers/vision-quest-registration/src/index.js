const SESSION_ORDER = ["Intro Talk", "Day Quest", "Day Quest Waitlist"];
const SESSION_SET = new Set(SESSION_ORDER);
const PUBLIC_SESSION_ORDER = ["Day Quest", "Day Quest Waitlist"];
...
function adminSessionLabels(row) {
  ...
  for (const session of sessions) {
    if (!["Day Quest Waitlist", "Day Quest", "Intro Talk"].includes(session)) {
...
function validateRegistration(payload) {
  ...
  if (sessions.some((session) => FULL_PUBLIC_SESSION_SET.has(session))) {
    throw new PublicError("The July 19 Day Quest is full. Please join the waitlist for the next available Day Quest.", 409);
  }
...
function buildRegistrantEmail(registration, env) {
  ...
  const title = hasWaitlist && !hasDay
